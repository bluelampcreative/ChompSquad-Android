...
- [x] **0.0** `Setup` — Version catalog (`libs.versions.toml`) — pin versions and declare aliases for every library in the stack above: Koin 4, Ktor Client 3 (CIO engine, content-negotiation, auth, logging), kotlinx.serialization, Jetpack Navigation 3, Room + KSP, DataStore (Proto), Coil 3, CameraX, Firebase BOM (Crashlytics, Analytics, App Distribution, FCM), RevenueCat, Material 3 Expressive, Compose BOM; add KSP plugin alias
- [x] **0.1** `Design` — ~~Receive and review brand artwork~~ **Artwork received, analysed, and decisions locked.** Assets confirmed: (a) **App icon** — chef-dad head on radial-gradient green; two variants: clean (production) and "D" letterform overlay (debug builds only, paired with `applicationId` suffix `.debug`). Follow Android adaptive icon spec: 108dp × 108dp artboard, foreground content within centred 66dp safe-zone circle. Minimum display: 48dp touch target per Material 3 guidelines. (b) **Logotype** — "Chomp Squad" italic display type on transparent background, layered dark-green shadow → white inline stroke → light-green fill; use as image asset (`logotype.svg`/`logotype.png`) — do not replicate in Compose `Text`. (c) **Squad illustration** — splash screen only; dad + two daughters composited on brand-green background.
...
- [ ] **0.3** `Setup` — Configure Android project baseline — min SDK 29, target/compile SDK 36, KSP enabled (Room only), wire Koin 4 via compiler plugin (`io.insert-koin.compiler.plugin`) — `startKoin { androidContext(this@App) }` in `Application` subclass, annotate modules with `@Module @ComponentScan`; `kotlin.serialization` and `kotlin.compose` plugins confirmed
...
- [ ] **1.2** `Auth` — Sign in with Google — `POST /v1/auth/google` confirmed in updated schema. Request: `GoogleAuthRequest { id_token: String }` (note: `id_token`, not `identity_token`). Response: `TokenResponse`. Use Credential Manager API to obtain Google ID token; `POST` via Ktor; store token pair in Proto DataStore (task 1.4).
